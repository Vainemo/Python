...
# 导入模块 一个模块只会被导入一次，不管你执行了多少次 import。这样可以防止导入模块被一遍又一遍地执行。
#Python的搜索路径，搜索路径是由一系列目录名组成的，Python 解释器就依次从这些目录中去寻找所引入的模块。
import support
# 现在可以调用模块里包含的函数了
support.print_func("Runoob")
import sys
print(sys.path)
#from … import 语句:从模块中导入一个指定的部分到当命名空间中
#from … import * 语句:把一个模块的所有内容全都导前入到当前的命名空间
#模块除了方法定义，还可以包括可执行的代码。这些代码一般用来初始化这个模块。这些代码只有在第一次被导入时才会被执行。
# 每个模块有各自独立的符号表，在模块内部为所有的函数当作全局符号表来使用。
from fibo import fib, fib2
fib(500)
#----------------
#__name__属性：个模块被另一个程序第一次引入时，其主程序将运行。如果我们想在模块被引入时，模块中的某一程序块不执行，
...
if __name__ == '__main__':
   ...
   print('我来自另一模块')
from fibo import test
test()
#dir() 函数以找到模块内定义的所有名称。以一个字符串列表的形式返回
import fibo
print(dir(fibo))
#从包中导入模块注意事项：1.尽量避免使用from sound.effects import *，因为当包中没有定义__all__ ，就不会导入包 sound.effects 里的任何子模块
# 只是把包sound.effects和它里面定义的所有内容导入进来（可能运行__init__.py里定义的初始化代码）。
# 这会把 __init__.py 里面定义的所有名字导入进来。并且他不会破坏掉我们在这句话之前导入的所有明确指定的模块
#---------------------------------------类--------------------------------------------------------
#-------------------------------------------------------------------------------------------------
#1.在Python中定义一个类时，括号中的参数代表继承了哪些父类，如：
#  class MyModel(Model):表示MyModel类继承了Model类
#构造函数__init__(self):
#  1.self它用于引用类的当前实例,
